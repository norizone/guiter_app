...
import { codeNameSets } from "@/features/code/stores/CodeNameSets";
import { noteKeySets } from "@/stores/NoteKeySets";
import { mq, size } from "@/theme/cssFunctions";
import { CodeFretBoard } from "@/features/code/components/CodeFretBoard";

import { typeCodeJson } from "@/types/components";
import {
  codeSelectedCode,
  codeSelectedKey,
} from "@/features/code/stores/CodeValues";
...
export const CodeFretBoardContents = () => {
  ...
  >([]);
  const [allStringPositions, setAllStringPositions] = useState<
  ...
  useEffect(() => {
    ...
    const getCodeValues = async () => {
      await axios(options)
        .then((res: AxiosResponse<Array<typeCodeJson>>) => {
  ...
  useEffect(() => {
    ...
    for (let stringIndex = 6; stringIndex >= 4; stringIndex--) {
      targetKeyPositions[stringIndex] = allStringPositions[
        stringIndex
      ].findIndex((el) => el === selectedNoteKeyIndex);
    }
    ...
    selectedCodeData.map((data) => {
      ...
      for (let stringIndex = 6; stringIndex >= 4; stringIndex--) {
        if (data.rootString === stringIndex) {
          ...
          const numberValue = proceedPosition.filter(
            (v): v is number => typeof v === "number"
          );
          const minPosition =
            numberValue.length !== 0 && Math.min(...numberValue);
          ...
          const removeBarreCode = minDuplicates
            ? numberValue.filter((el) => el !== minDuplicates)
            : [];
          removeBarreCode.length > 0 && removeBarreCode.push(minDuplicates);

          // 値にマイナスを含むもの&&物理的に押せない5本指以上のコードはスルー
          if (
            proceedPosition.every((el) => Number(el) > -1) &&
    ...
    newProcessedCode.length > 1 &&
      newProcessedCode.sort((a, b) => {
        return Math.max(Number(...a)) - Math.max(Number(...b));
      });
    setProcessedCode(newProcessedCode);
  }, [
    selectedCodeData,
    selectedNoteKeyIndex,
    allStringPositions,
    selectedTuning,
  ]);

  return (
    <div css={[slider, processedCode.length !== 0 && sliderActive]}>
      <Swiper
        modules={[Navigation]}
        spaceBetween={0}
        slidesPerView={1}
      ...
      >
        {processedCode?.map((codeData, index) => (
          <SwiperSlide key={index}>
            <CodeFretBoard
              processedCode={codeData}
              allStringPositions={allStringPositions}
            />
          </SwiperSlide>
        ))}
      </Swiper>
