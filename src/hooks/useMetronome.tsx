...
export const useMetronome = () => {
  ...
  const [thisBeat, setThisBeat] = useState(
    metronomeBeats[selectedMetroBeat].value
  );
  ...
  const onPlayMetronome = () => {
    ...
    setEventId(
      Tone.Transport.scheduleRepeat((time) => {
        thisBeat[thisCount] === 2 &&
          metronome.triggerAttackRelease("A5", "32n", time);
        thisBeat[thisCount] === 1 &&
          metronome.triggerAttackRelease("C5", "32n", time);
    ...
    return (
      <div css={mCountWrap} ref={countRef}>
        {metronomeBeats[selectedMetroBeat].value.map((_, index) => (
          <span css={[mCount]} key={index}></span>
        ))}
      </div>
    );
  };
