...
const setBpmStates = (event:React.ChangeEvent<HTMLInputElement>)=>{
  ...
  debounce(()=>{
    ...
    ? newValue : newValue <= minNum ? minNum : maxNum;
  ...
  return (
    <div css={[bpmWrap]}>
		<p>
			<span>BPM</span>
			<input type="number" value={inputBpm}
      onChange={(event) => setBpmStates(event)}
      css={[bmpInput]}  aria-label="BPM変更" 
      ...
      `--bar-width:${bpmBar}%`
			...
			<input id="bpmBar"  type="range" value={inputBpm}
      onChange={(event) => setBpmStates(event)}  aria-label="BPM変更" 
      min={minNum} max={maxNum}
      step="1"
      />
		</div>
