...
export const SelectRhythmTypeArea = () => {
  ...
  return (
    <div css={[
      selectRhythmTypeStyle,
      rhythmType&& `--btn-position:50%`]}>
      <button type="button" onClick={()=>setRhythmType(0)} 
      css={[rhythmBtn,!rhythmType&&btnActive ]}
      disabled={!rhythmType ? true : false}>Metronome</button>
      <button type="button" onClick={()=>setRhythmType(1)} 
      css={[rhythmBtn,rhythmType&&btnActive]}
      disabled={rhythmType ? true : false}>Drams</button>
    </div>
  )
}
...
const selectRhythmTypeStyle = css`
        ...
        border-radius: ${size.vw(375, 30)};
        z-index: 0;
        transition: left .2s cubic-bezier(0.215, 0.610, 0.355, 1);
    }
    `
