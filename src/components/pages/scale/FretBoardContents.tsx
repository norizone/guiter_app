...
import { FretBoard } from "@/components/pages/scale/FretBoard"
...
export const FretBoardContents = () =>{
  ...
  const [fretNumbers] = useState<Array<number>>(Array.from({ length: 25 }, (_, index) => index));
  ...
  return(
    ...
    <table >
      <tbody>
        <tr css={[numberRow , 
          tunings[selectedTuningIndex].value.length === 4 && dotBasePosition,
          tunings[selectedTuningIndex].value.length === 6 && dotPrimaryPosition ]}>
          {fretNumbers.map((_, index) => (
            <td 
              css={[ 
                numberBox,
                (index%2 !==0 && noDotPositions.every(d=>d!==index)) && numberRowOneDot,
                (index === 12 || index === 24) && numberRowTowDot]}
              key={index}>{index>0 && index}</td>
          ))}
        </tr>
      </tbody>
    </table>
