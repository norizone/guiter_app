          ...
          {processedCode.map((position, rowIndex) => (
            <Fragment key={rowIndex} >
              <tr css={[
                ...
                barreFirstIndex>-1 && barreFirstIndex===rowIndex && barreCodeStyle,
                css` 
                --start-gradient: ${rowIndex * .05} ;
                --end-gradient:${(rowIndex + 1) * .05};`
                ]}>
                {Array(maxFrets).fill('').map((_, boxIndex) => {
                  return(
                  <td key={boxIndex} css={[
                    primaryBox,
                    (boxIndex+1 - maxFrets) + maxPosition === 0 && openBox,
                    (boxIndex+1 - maxFrets) + maxPosition === position && activeBox,
                    typeof position=== "boolean" && position === false  && muteBoxMute,
                    ]}
                    >
                    {(boxIndex+1 - maxFrets) + maxPosition === position ? 
                    typeof position !== "boolean" && noteKeys[allStringPositions[rowIndex+1][position]]?.name :''}
                  </td>
                  )
                  }
                )}
              ...
              {processedCode.length===rowIndex+1&&
                <tr css={[
                  baseRow,
                  processedCode.length===6 && string6
                ]}>
                  {Array(maxFrets).fill('').map((_, numIndex) => (
                    <td key={numIndex}
                      css={[
                        numberBox,
                        ((numIndex+1 - maxFrets) + maxPosition) === 12  && doubleDotRow,
